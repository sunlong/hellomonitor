...
<head>
    ...
    <script id="addUserGroupTmpl" type="text/html">
        <div id="error"></div>
    ...
    <script type="text/javascript">
        $(function(){
            function beforeDrop(treeId, treeNodes, targetNode) {
                ...
                if(targetNode){
                    $.ajax({
                        ...
                        type: 'POST',
                        dataType : "json",
                        url: '${ctx}/userGroup/changeParent',
            ...
            <shiro:hasPermission name="userGroup:create">
                showAddBtn = true;
            </shiro:hasPermission>
            ...
            var setting = {
                view: {
                    ...
                    addHoverDom: function addHoverDom(treeId, treeNode) {
                        ...
                        var addStr = "<span class='button add' id='addBtn_" + treeNode.value + "' title='添加子节点' onfocus='this.blur();'></span>";
                        sObj.after(addStr);
                        var btn = $("#addBtn_"+treeNode.value);
                        if (btn) {
                            btn.bind("click", function(){
                                $("input[name='parentUserGroup.id']").val(treeNode.value);
            ...
            var getConfig = function(formId, errorId){
                return {
                    ...
                    submitHandler: function(){
                        $.post('${ctx}/userGroup/create', $(formId).serialize(), function(data){
                            if(data.success){
                                location.href = '${ctx}/userGroup/list';
                            }else{
            ...
            $("#add-root").on('click', "button", function(){
                $("#add-root").html(parseTemplate($("#addUserGroupTmpl").html(), {}));
                //客户端验证
                $("#add-root-form").validate(getConfig('#add-root-form', '#error'));
            });
            $("#add-root").on('click', "input[type='button']", function(){
                $("#add-root").html('<button class="btn btn-primary">添加根用户组</button>');
            });
            //~
...
<div class="container-fluid">
    <div class="row-fluid">
        <%@ include file="/common/user/userGroup-sidebar.jsp"%>
        <div class="span10 content"><!-- content starts -->
            ...
            <div class="well well-large">
                ...
                <div id="add-root">
                    <shiro:hasPermission name="userGroup:create">
                        <button class="btn btn-primary">添加根用户组</button>
                    </shiro:hasPermission>
                </div>
...
<div class="modal hide fade" id="addUG">
    ...
    <form class="form-horizontal" id="add-form" autocomplete="off">
        <div class="modal-body">
            <input type="hidden" name="parentUserGroup.id"/>
